...
public class ConfirmEventFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = inflater.inflate(R.layout.fragment_confirm_event, container, false);
    ...
    private void getEventTrainer(){
        ...
        AndroidNetworking.get("https://testapi.digitalevent.id/lms/api/myevent/trainer?tr_id=2&stat_relat=ST01")
                            ...
                            if (jsonArray.length()==0) {
                                ...
                                listActivity.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                                    ...
                                    public void onItemClick(AdapterView<?> parent, View view, final int position, long id) {
                                        String begda = listModelTrainer.get(position).getBEGDA();
                                        String buscd = listModelTrainer.get(position).getBEGDA();
                                        String child = listModelTrainer.get(position).getBEGDA();
                                        String endda = listModelTrainer.get(position).getBEGDA();
                                        String objid = listModelTrainer.get(position).getBEGDA();
                                        String ochil = listModelTrainer.get(position).getBEGDA();
                                        String opare = listModelTrainer.get(position).getBEGDA();
                                        String paren = listModelTrainer.get(position).getBEGDA();
                                        String relat = listModelTrainer.get(position).getBEGDA();
                                        popupApprove(begda,buscd,child,endda,objid,ochil,opare,paren,relat);
                                    }
                                });
                            }
    ...
    private void popupApprove(String begda, String buscd, String child, String endda, String objid, String ochil, String opare, String paren, String relat) {
        ...
        btnYes.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                postApprove(begda, buscd, child, endda, objid, ochil, opare, paren, relat);

            }
        });
    }

    private void postApprove (String begda, String buscd, String child, String endda, String objid, String ochil, String opare, String paren, String relat) {
        JSONObject jsonObject = new JSONObject();
        try {
            jsonObject.put("begin_date", begda);
            jsonObject.put("business_code", buscd);
            jsonObject.put("child", child);
            jsonObject.put("end_date", endda);
            jsonObject.put("object_identifier", objid);
            jsonObject.put("otype_child", ochil);
            jsonObject.put("otype_parent", opare);
            jsonObject.put("parent", paren);
            jsonObject.put("relation", relat);
        } catch (JSONException e) {
            e.printStackTrace();
        }

        AndroidNetworking.put("https://testapi.digitalevent.id/lms/api/lmsrelation")
